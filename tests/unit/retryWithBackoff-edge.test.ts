...
describe("retryWithBackoff edge cases", () => {
  ...
  it("handles zero maxRetries (no retry)", async () => {
    ...
    await expect(retryWithBackoff(fn, { maxRetries: 0, baseDelayMs: 10 })).rejects.toThrow("500");
  ...
  it("does not retry on 403 forbidden", async () => {
    ...
    await expect(retryWithBackoff(fn, { maxRetries: 2, baseDelayMs: 10 })).rejects.toThrow("403");
    expect(fn).toHaveBeenCalledTimes(1);
  });
