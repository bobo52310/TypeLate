...
透過可自訂的全域快捷鍵，在任何應用程式觸發語音輸入。支援 **按住**、**切換** 和 **雙擊** 三種觸發模式。

### 口語轉書面語

AI 自動移除贅字、重組句子結構、修正標點符號。提供三種增強模式：
...
- **自訂詞彙** — 教 TypeLate 你的專有名詞、技術術語和行話，提升轉錄準確度。支援批次匯入。
...
你的 API Key 直接送往 Groq — 語音資料不經過我們的伺服器。所有轉錄和錄音皆儲存在你的裝置本地。
